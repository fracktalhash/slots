...
SYMBOL_WEIGHTS_3x4 = {
    '♥': 0.3, '♠': 0.3, 'Δ': 0.1, 'Ω': 0.1, 'Ψ': 0.1,
    '♝': 0.025, '♞': 0.025, '♜': 0.005, '♛': 0.005, '♚': 0.002
}

SYMBOL_WEIGHTS_3x3 = {
    '♥': 0.13, '♠': 0.13, 'Δ': 0.13, 'Ω': 0.13, 'Ψ': 0.07,
...
pay_table = {
    '♥': {3: 2, 4: 4}, '♠': {3: 2, 4: 4},
    'Δ': {3: 5, 4: 10}, 'Ω': {3: 5, 4: 10}, 'Ψ': {3: 5, 4: 10},
    '♝': {3: 15, 4: 30}, '♞': {3: 15, 4: 30},
    '♜': {3: 50, 4: 100}, '♛': {3: 50, 4: 100}, '♚': {3: 100, 4: 200}
}
...
class Slot:
    ...
    def __init__(self, select: str ):
        ...
        self.slot_size = self.config(select)  # Initialize slot size
    ...
    def config(select):
        ...
        while True:
            ...
            if select in valid_selection:
                ...
                select = input("Invalid selection. Please select 'a' for 3x3 or 'b' for 3x4: ").strip().lower()
            
    def spin(self):

        spun_slot = copy.deepcopy(self.slot_size)

        for row in range(len(self.slot_size)):

            for col in range(len(self.slot_size[0])):

                symbol = random.choice(self.reels[col])
    
                spun_slot[row][col] = symbol
...
class Reel:
    ...
    def __init__(self, slot_size : list):

        self.slot_size = slot_size

        self.reel_config = self.configure_reel()

    def configure_reel(self):

        if len(self.slot_size[0]) == 4:
...
def calculate_payout(winning_lines, pay_table, line_bonus, bet=10):
    total = 0
    for line in winning_lines:
        ...
        total += payout
        console.print(f"[green]{line['line_type'].title()} line:[/] {line['symbol']} x{line['match_count']} → {base}x * {bonus} = {payout}")
    return total
